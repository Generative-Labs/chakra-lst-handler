...
import {ISettlementSignatureVerifier} from "../../src/interfaces/ISettlementSignatureVerifier.sol";
 ...
 */
contract MockSettlementSignatureVerifier is
    OwnableUpgradeable,
    UUPSUpgradeable,
    ...
    ISettlementSignatureVerifier
{
    ...
    event RequiredValidatorsChanged(
        address indexed manager, uint256 indexed old_required_validators, uint256 indexed new_required_validators
    );
    ...
    function verifyECDSA(bytes32 msgHash, bytes calldata signatures) internal view returns (bool) {
        ...
        for (uint256 i = 0; i < len; i += 65) {
            ...
            console.log("is_validator", validators[msgHash.recover(sig)]);
